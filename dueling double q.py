...
def train_dddqn(env):
    ...
    class Q_Network(chainer.Chain):
        ...
        def __call__(self, x):
            ...
            advantage_mean = (F.sum(advantage_value, axis=1) / float(
                self.output_size)).reshape(-1, 1)
            q_value = F.concat([state_value for _ in range(self.output_size)],
                               axis=1) + (advantage_value - F.concat(
                ...
                if total_step % train_freq == 0:
                    ...
                    for i in memory_idx[::batch_size]:
                        ...
                        for j in range(batch_size):
                            ...
                            === """
                            target[j, b_pact[j]] = b_reward[j] + gamma * maxqs[
                                j, indices[j]] * (not b_done[j])
                            """ >>> """
                        Q.reset()
