...
def plot_train_test(train, test, date_split):
    data = [
        go.Candlestick(x=train.index, open=train['Open'], high=train['High'],
                    # low=train['Low'], close=train['Close'], name='train'),
        go.Candlestick(x=test.index, open=test['Open'], high=test['High'],
...
class Environment1:
    ...
    def step(self, act):
        ...
        return [
                   self.position_value] + self.history, reward, self.done  # obs, reward, done
...
def train_dqn(env):
    ...
    for epoch in range(epoch_num):
                ...
                if total_step % train_freq == 0:
                    ...
                    for i in memory_idx[::batch_size]:
                        ...
                        for j in range(batch_size):
                            target[j, b_pact[j]] = b_reward[j] + gamma * maxq[
                                j] * (not b_done[j])
                        Q.reset()
        ...
        if (epoch + 1) % show_log_freq == 0:
            ...
            print('\t'.join(map(str,
                                [epoch + 1, epsilon, total_step, log_reward,
...
def plot_loss_reward(total_losses, total_rewards):
    figure = tools.make_subplots(rows=1, cols=2,
                                 ...
                                 print_grid=False)
    figure.append_trace(
        go.Scatter(y=total_losses, mode='lines', line=dict(color='skyblue')), 1, 1)
    figure.append_trace(
        go.Scatter(y=total_rewards, mode='lines', line=dict(color='orange')), 1, 2)
